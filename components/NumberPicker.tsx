...
export default function NumberPicker({ 
  ...
  const [divisionStats, setDivisionStats] = useState<Record<string, number>>({});
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (user) {
        ...
        if (data) {
            setWinningHistory(data.flatMap(item => {
                const innerLines = Array.isArray(item.lines) ? item.lines : [item.lines];
                return innerLines.map((l: any) => ({
                    ...l,
                    drawNumbers: l.drawNumbers || [],
                    drawBonus: l.drawBonus || []
                }));
            }));
        }
      } else {
        ...
        if (storedWins) {
            ...
            setWinningHistory(parsed.map((l: any) => ({
                ...l,
                drawNumbers: l.drawNumbers || [],
                drawBonus: l.drawBonus || []
            })));
    ...
    timerRef.current = setInterval(() => {
      ...
      currentTickets.forEach((ticketNumbers) => {
        ...
        if (result.prizeTier !== "No Prize") {
          ...
          weeklyDivStats[result.prizeTier] = (weeklyDivStats[result.prizeTier] || 0) + 1;
      ...
      setDivisionStats(prev => {
          const updated = { ...prev };
          Object.keys(weeklyDivStats).forEach(tier => {
              updated[tier] = (updated[tier] || 0) + weeklyDivStats[tier];
          });
          return updated;
      });

      if (newWinners.length > 0) setAutoWinners(prev => [...newWinners, ...prev].slice(0, 500));
    }, interval);
  };
        ...
        <div className="animate-in fade-in slide-in-from-right-4 duration-500">
          ...
          {!isRunning && stats.draws > 0 && (
            <div className={`bg-white dark:bg-gray-900 rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-10 border-2 ${brandStyles.border} shadow-2xl mb-8 sm:mb-10 animate-in zoom-in-95 duration-500 text-left`}>
                        ...
                        <div className="space-y-1.5 sm:space-y-2">
                            {Object.entries(divisionStats).sort((a,b) => parseInt(a[0].replace(/\D/g,'')) - parseInt(b[0].replace(/\D/g,''))).map(([tier, count]) => (
                                <div key={tier} className="flex justify-between items-center bg-gray-50 dark:bg-white/5 px-3 py-1.5 sm:px-4 py-2 rounded-lg sm:rounded-xl">
                                    <span className="text-[10px] sm:text-xs font-bold text-gray-600 dark:text-gray-400">{tier}</span>
                                    <span className={`text-xs sm:text-sm font-black text-gray-900 dark:text-white`}>{count}x</span>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <button onClick={() => { setStats({ draws: 0, spent: 0, won: 0 }); setDivisionStats({}); setAutoWinners([]); }} className="w-full py-3.5 sm:py-4 rounded-xl sm:rounded-2xl bg-gray-100 dark:bg-white/5 text-gray-400 font-black uppercase text-[9px] sm:text-[10px] tracking-[0.3em] hover:bg-gray-200 transition-all">Reset Simulation</button>
            </div>
          )}
          ...
          <div className="bg-white dark:bg-gray-900 rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-10 border border-gray-100 dark:border-white/5 shadow-xl mb-8 sm:mb-10 space-y-8 sm:space-y-10 text-left">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-10">
                ...
                <div className="space-y-4 sm:space-y-6">
                    <p className="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Warp Speed</p>
                    <div>
                        <label className="block text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-3 sm:mb-4">Speed: {drawsPerSec} draws/sec</label>
