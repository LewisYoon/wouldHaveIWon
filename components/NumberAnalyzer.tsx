...
export default function NumberAnalyzer() {
  ...
  const analysis = useMemo(() => {
    ...
    return { evenCount, oddCount, sum, lowCount, highCount, sumScore };
  }, [numbers]);

  const handleProcessInput = () => {
    const parsed = inputVal.split(/[, ]+/).map(n => parseInt(n.trim())).filter(n => !isNaN(n) && n > 0 && n <= 47);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-10 bg-white dark:bg-gray-900 rounded-[3rem] border border-gray-100 dark:border-white/5 shadow-2xl text-left">
      <h2 className="text-3xl font-black mb-8 uppercase tracking-tighter italic border-b-4 border-indigo-500 inline-block text-gray-900 dark:text-white">Lucky Number Analyzer</h2>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        <div className="space-y-6">
          <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Enter your numbers separated by space or comma to analyze their mathematical balance.</p>
          ...
          <div className="flex flex-wrap gap-3">
            {numbers.map(n => (
              <span key={n} className="w-12 h-12 rounded-full bg-indigo-50 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 flex items-center justify-center font-black text-lg border border-indigo-100 dark:border-indigo-500/20">{n}</span>
            ))}
          </div>
        </div>
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
              ...
              <div className="pt-6 border-t border-gray-200 dark:border-white/10">
                <p className="text-xs text-gray-500 dark:text-gray-400 font-medium leading-relaxed italic">
                  *Statistically, most winning combinations have a balanced 3:3 or 4:2 ratio of odd/even and low/high numbers. This analysis provides a mathematical snapshot, not a prediction.
                </p>
              </div>
            </div>
          ) : (
