...
function LuckContent() {
  ...
  const handleNumberClick = (n: number) => {
    const max = game === 'Powerball' ? 7 : game === 'Oz Lotto' ? 7 : 6;
    if (currentNumbers.includes(n)) {
      setCurrentNumbers(currentNumbers.filter(num => num !== n));
    } else if (currentNumbers.length < max) {
  ...
  const handleSaveTicket = async () => {
    ...
    if (currentNumbers.length < (game === 'Powerball' ? 7 : required)) {
      alert(`Please select ${game === 'Powerball' ? 7 : required} numbers.`);
      return;
    }
  ...
  const handleQuickPick = () => {
    setCurrentNumbers(generateQuickPick(game));
  };
  ...
  const handleMultiQuickPick = async () => {
    ...
    if (!isPremium) {
      ...
      if (availableSpace <= 0) {
        alert("Free tier limit reached. Upgrade to PRO for unlimited tracking!");
        return;
      }
      if (quickPickQty > availableSpace) {
        alert(`Only ${availableSpace} slots remaining for this draw on the Free Tier.`);
        return;
      }
    }

    const tickets = Array.from({ length: quickPickQty }, () => ({
      user_id: user.id,
      draw_date: selectedDate,
    ...
    <div className="flex flex-col min-h-screen bg-white dark:bg-gray-950 selection:bg-indigo-500 selection:text-white transition-colors duration-500">
      ...
      <main className="flex-grow max-w-7xl mx-auto w-full px-4 sm:px-6 py-10 sm:py-16">
          ...
          <div className="flex bg-gray-100 dark:bg-white/5 p-1.5 rounded-[2rem] border border-gray-200 dark:border-white/10 w-full md:w-auto overflow-x-auto no-scrollbar">
            {(['Oz Lotto', 'Powerball', 'Tatts Lotto'] as const).map((g) => (
              <button
                ...
                className={`flex-1 md:flex-none px-6 sm:px-10 py-3 sm:py-4 rounded-[1.5rem] text-xs sm:text-sm font-black uppercase tracking-widest transition-all duration-500 whitespace-nowrap ${
                  game === g 
        ...
        <div className="grid lg:col-span-12 grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12">
          ...
          {/* Left Column: Number Picker & Selection */}
                ...
                {selectedJackpot && (
                  <div className="text-right">
                    ...
                    <p className="text-xl font-black text-gray-900 dark:text-white tracking-tighter">
                      ${(selectedJackpot / 1000000).toFixed(0)}M
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-4 no-scrollbar">
                {upcomingDates.map((date) => (
                  <button
                    ...
                    className={`flex-shrink-0 px-5 sm:px-6 py-3 sm:py-4 rounded-2xl sm:rounded-3xl border-2 transition-all font-black text-[10px] sm:text-xs uppercase tracking-widest ${
                      selectedDate === date 
                        ? `${brandStyles.border} ${brandStyles.text} ${brandStyles.bgLight} scale-105` 
                ...
                <div className="mt-10 pt-8 border-t border-gray-50 dark:border-white/5">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="h-1.5 bg-gray-100 dark:bg-white/5 rounded-full flex-grow overflow-hidden">
                      <div 
                        className={`h-full transition-all duration-1000 ${brandStyles.bg}`}
          ...
          <div className="lg:col-span-7 space-y-8 sm:space-y-10 animate-in fade-in slide-in-from-right-8 duration-1000 text-left">
              ...
              <div className="flex justify-between items-start mb-8 sm:mb-10">
                ...
                {!isPremium ? (
                  ...
                  <button 
                    ...
                    className={`w-12 h-7 rounded-full transition-colors relative flex-shrink-0 ${((autoTrackGames[game] || 0) > 0) ? 'bg-amber-500' : 'bg-gray-200 dark:bg-gray-800'}`}
                  >
                    <div className={`absolute top-1 left-1 w-5 h-5 bg-white rounded-full transition-transform shadow-sm ${((autoTrackGames[game] || 0) > 0) ? 'translate-x-5' : 'translate-x-0'}`} />
                  </button>
                )}
              </div>
              ...
              {isPremium && (
                <div className="space-y-8 animate-in slide-in-from-top-2 duration-500">
                  ...
                  <div className={`p-4 rounded-2xl border transition-colors ${((autoTrackGames[game] || 0) > 0) ? 'bg-amber-50 dark:bg-amber-500/5 border-amber-100 dark:border-amber-500/10' : 'bg-gray-50 dark:bg-white/5 border-transparent'}`}>
                    <p className={`text-[9px] font-medium leading-relaxed italic text-left ${((autoTrackGames[game] || 0) > 0) ? 'text-amber-700 dark:text-amber-400' : 'text-gray-400'}`}>
                      {((autoTrackGames[game] || 0) > 0) 
                        ? `We will automatically generate ${autoTrackGames[game]} random tickets for you as soon as new results are announced.`
                        : `Auto-Tracker is currently disabled for ${game}. Toggle the switch above to enable.`}
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className="bg-white dark:bg-gray-900 rounded-[2.5rem] sm:rounded-[3rem] p-6 sm:p-10 border border-gray-100 dark:border-white/5 shadow-xl">
              <div className="flex justify-between items-center mb-8 sm:mb-10">
                <div className="text-left">
                  <h3 className="text-lg sm:text-xl font-black text-gray-900 dark:text-white uppercase tracking-tighter italic">Your Archive</h3>
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Tracked Ticket History</p>
                </div>
                <button onClick={() => setIsRulesModalOpen(true)} className="p-2 text-gray-400 hover:text-indigo-500 transition-colors">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-10 sm:mb-12">
                {[
                  ...
                  <div key={i} className="bg-gray-50 dark:bg-white/5 p-4 rounded-2xl text-center border border-transparent hover:border-gray-100 dark:hover:border-white/10 transition-all">
                    <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">{s.label}</p>
                    <p className={`text-sm sm:text-base font-black uppercase tracking-tighter ${s.color}`}>{s.val}</p>
                  </div>
                ))}
              </div>
              ...
              {isDataLoading ? (
                <div className="py-20 text-center animate-pulse">
                  <p className="text-gray-400 font-bold uppercase tracking-[0.3em] text-[10px]">Syncing Archive...</p>
                </div>
              ) : myTickets.length === 0 ? (
                <div className="py-20 sm:py-32 text-center">
                  ...
                  <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px]">No tickets tracked yet</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {/* Grouped by Date */}
                    ...
                    return (
                      <div key={date} className="border border-gray-50 dark:border-white/5 rounded-[1.5rem] sm:rounded-[2rem] overflow-hidden bg-gray-50/30 dark:bg-white/5 transition-all">
                        <button 
                          ...
                          className="w-full flex items-center justify-between p-5 sm:p-6 hover:bg-gray-100/50 dark:hover:bg-white/5 transition-colors"
                        >
                          <div className="flex items-center gap-4">
                            <div className={`w-2 h-2 rounded-full ${result ? 'bg-emerald-500 animate-pulse' : 'bg-gray-300'}`} />
                            <div className="text-left">
                              <p className="text-xs sm:text-sm font-black text-gray-900 dark:text-white uppercase tracking-tighter">{date}</p>
                              <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">{ticketsForDate.length} {ticketsForDate.length === 1 ? 'Sequence' : 'Sequences'}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-4">
                            {result && (
                              <span className="text-[9px] font-black text-emerald-500 uppercase tracking-widest hidden sm:block">Results Ready</span>
                            )}
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className={`text-gray-400 transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`}><polyline points="6 9 12 15 18 9"></polyline></svg>
                          </div>
                        </button>
                              ...
                              return (
                                <div key={ticket.id} className="flex items-center justify-between p-4 bg-white dark:bg-gray-900 rounded-2xl border border-gray-50 dark:border-white/5 group shadow-sm">
                                      ...
                                      return (
                                        <span 
                                          ...
                                          className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-[10px] sm:text-xs font-black transition-all ${
                                            isMatch ? `${brandStyles.bg} text-white shadow-lg` : 
                                            isBonusMatch ? 'bg-amber-400 text-amber-950 shadow-lg' : 
                                  ...
                                  <div className="flex items-center gap-3">
                                    {comparison && (
                                      <span className={`text-[9px] font-black uppercase tracking-widest ${comparison.prizeTier !== 'No Prize' ? 'text-emerald-500' : 'text-gray-300'}`}>
                                        {comparison.prizeTier === 'No Prize' ? 'Missed' : comparison.prizeTier.replace('Division ', 'Div ')}
                                      </span>
                                    )}
                                    <button 
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-gray-950">
        <p className="text-sm font-black text-gray-400 uppercase tracking-widest animate-pulse">Initializing Luck Tracker...</p>
      </div>
    }>
      <LuckContent />
    </Suspense>
